...
struct TestMemOverride {
    read_block: Option<fn (&mut TestMem, address: u32, length: usize) -> core::result::Result<&[u8], DFUMemError>>,
    erase_block: Option<fn (&mut TestMem, address: u32) -> Result<(), DFUMemError>>,
    program_block: Option<fn (&mut TestMem, address: u32, length: usize) -> core::result::Result<(), DFUMemError>>,
    manifestation: Option<fn (&mut TestMem) -> Result<(), DFUManifestationError>>,
}

impl TestMem {
    fn new(overrides:Option<TestMemOverride>) -> Self {
        let tmo = overrides.unwrap_or(TestMemOverride{read_block:None,erase_block:None,program_block:None,manifestation:None});
        Self {
...
fn test_get_configuration() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        let u16v: Vec<_> = buf[2..len].chunks(2).map(|v| (v[0] as u16) | ((v[1] as u16) << 8)).collect();
...
fn test_set_address_pointer() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        len = transact(&mut dfu, &[0x21, 0x1, 0,0, 0,0, 5,0], Some(&[0x21, b[0], b[1], b[2], b[3]]), &mut buf).expect("len");
        assert_eq!(len, 0);
...
fn test_block_erase() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        len = transact(&mut dfu, &[0x21, 0x1, 0,0, 0,0, 5,0], Some(&[0x41, b[0], b[1], b[2], b[3]]), &mut buf).expect("len");
        assert_eq!(len, 0);
...
fn test_block_erase_all() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        len = transact(&mut dfu, &[0x21, 0x1, 0,0, 0,0, 1,0], Some(&[0x41]), &mut buf).expect("len");
        assert_eq!(len, 0);
        ...
        loop {
            /* Upload block - erased */
            len = transact(&mut dfu, &[0xa1, 0x2, (blk & 0xff) as u8, (blk >> 8) as u8, 0,0, 128,0], None, &mut buf).expect("len");
...
fn test_block_upload_last() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        len = transact(&mut dfu, &[0xa1, 0x2, 0x01,0x2, 0,0, 128,0], None, &mut buf).expect("len");
        assert_eq!(len, 128);
        assert_eq!(&buf[0..10], &[192,127, 193,127, 194,127, 195,127, 196,127]);
        ...
        len = transact(&mut dfu, &[0xa1, 0x2, 0x02,0x2, 0,0, 128,0], None, &mut buf).expect("len");
        assert_eq!(len, 0);
...
fn test_block_upload_err_bad_address() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        /* Upload block 2 (offset 0) */
        let e = transact(&mut dfu, &[0xa1, 0x2, 2,0, 0,0, 128,0], None, &mut buf).expect_err("stall");
...
fn test_block_download_program0() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        len = transact(&mut dfu, &[0x21, 0x1, 2,0, 0,0, 128,0], Some(&[0;128]), &mut buf).expect("len");
        assert_eq!(len, 0);
...
fn test_block_download_program_err_verify_and_to_idle() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        /* Download block 2 (offset 0) */
        len = transact(&mut dfu, &[0x21, 0x1, 2,0, 0,0, 128,0], Some(&[0x55;128]), &mut buf).expect("len");
        assert_eq!(len, 0);
...
fn test_manifestation_no_reset() {
    with_usb(&mut MkDFUMTret{}, |mut dfu, transact| {
        ...
        /* Abort */
        let e = transact(&mut dfu, &[0x21, 0x6, 0,0, 0,0, 0,0], None, &mut buf).expect_err("stall");
        assert_eq!(e, EPErr::Stalled);
...
fn test_program_err_prog_write() {
    with_usb(&mut MkDFUProgerr{}, |mut dfu, transact| {
        ...
        len = transact(&mut dfu, &[0x21, 0x1, 3,0, 0,0, 128,0], Some(&[0x55;128]), &mut buf).expect("len");
        assert_eq!(len, 0);
...
impl ClsMaker<TestBus, DFUClass<TestBus, TestMem>> for MkDFUReadErr {
    fn create<'a>(&mut self, alloc: &'a UsbBusAllocator<TestBus>) -> DFUClass<TestBus, TestMem> {
        ...
        fn read_block(tm:&mut TestMem, address: u32, length: usize) -> core::result::Result<&[u8], DFUMemError> {
            if address > TestMem::INITIAL_ADDRESS_POINTER {
...
fn test_read_err_addr_vend() {
    with_usb(&mut MkDFUReadErr{}, |mut dfu, transact| {
        ...
        /* Upload block 2 (offset 0) */
        e = transact(&mut dfu, &[0xa1, 0x2, 2,0, 0,0, 128,0], None, &mut buf).expect_err("stall");
        assert_eq!(e, EPErr::Stalled);
        ...
        e = transact(&mut dfu, &[0xa1, 0x2, 3,0, 0,0, 128,0], None, &mut buf).expect_err("stall");
        assert_eq!(e, EPErr::Stalled);
...
fn test_block_download_program_short() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        len = transact(&mut dfu, &[0x21, 0x1, 2,0, 0,0, 16,0], Some(&[0xaa;16]), &mut buf).expect("len");
        assert_eq!(len, 0);
...
fn test_status_err_small_buffer() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        let e = transact(&mut dfu, &[0xa1, 0x3, 0,0, 0,0, 5,0], None, &mut buf).expect_err("stall");
        assert_eq!(e, EPErr::Stalled);
    });
}
...
fn test_state_err_small_buffer() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        let e = transact(&mut dfu, &[0xa1, 0x5, 0,0, 0,0, 0,0], None, &mut buf).expect_err("stall");
        assert_eq!(e, EPErr::Stalled);
    });
}
...
fn test_commands_err_small_buffer() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        let e = transact(&mut dfu, &[0xa1, 0x2, 0,0, 0,0, 2,0], None, &mut buf).expect_err("stall");
        assert_eq!(e, EPErr::Stalled);
    });
}
...
fn test_err_addr_overflow() {
    with_usb(&mut MkDFU{}, |mut dfu, transact| {
        ...
        let e = transact(&mut dfu, &[0xa1, 0x2, 3,0, 0,0, 128,0], None, &mut buf).expect_err("stall");
        assert_eq!(e, EPErr::Stalled);
