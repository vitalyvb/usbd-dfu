...
impl TestBusIO {
    fn new() -> Self {
        Self {
            ep_i: [RefCell::new(EP::new()),RefCell::new(EP::new()),RefCell::new(EP::new()),RefCell::new(EP::new())],
            ep_o: [RefCell::new(EP::new()),RefCell::new(EP::new()),RefCell::new(EP::new()),RefCell::new(EP::new())],
        }
    }
...
impl usb_device::bus::UsbBus for TestBus {
    ...
    fn alloc_ep(&mut self, _ep_dir: UsbDirection, ep_addr: Option<EndpointAddress>, _ep_type: EndpointType, _max_packet_size: u16, _interval: u8) -> Result<EndpointAddress> {
...
pub fn with_usb<T,M>(maker: &mut M, case: fn(dfu: &mut T, transact: &mut dyn FnMut(&mut T, &[u8], Option<&[u8]>, &mut [u8]) -> core::result::Result<usize, EPErr>))
    where T: UsbClass<TestBus>,
          M: ClsMaker<TestBus, T>
{
    ...
    let mut transact =  |d:&mut T, setup:&[u8], data:Option<&[u8]>, out: &mut [u8]| -> core::result::Result<usize, EPErr> {
        let out0 = EndpointAddress::from_parts(0, UsbDirection::Out);
        let in0 = EndpointAddress::from_parts(0, UsbDirection::In);
